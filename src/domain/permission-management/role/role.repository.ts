...
export abstract class RoleRepository implements BaseRepository<Role> {
  abstract findAll(q: string): Promise<Role[]>;
  ...
  abstract addRolesToUser(roleIds: EntityId[], userId: EntityId): void;
  abstract removeRolesFromUser(roleIds: EntityId[], userId: EntityId): void;
}
